<!-- SPDX-FileCopyrightText: 2026 Knitli Inc. -->
<!-- SPDX-License-Identifier: MIT OR Apache-2.0 -->

## Summary

<!-- A clear and concise description of what this PR does and why. -->

## Changes

<!-- List the key changes made in this PR. -->
<!-- Example:
- Updated X to support Y
-->
## Related Issues

<!-- Link any related issues here. Use "Closes #123" to auto-close on merge. -->

Closes #

## Testing

<!-- Describe how you tested your changes, including any relevant test cases. -->

- [ ] Tests added / updated
- [ ] Manually tested

## Checklist

- [ ] My changes follow the project's coding style and conventions
- [ ] I've updated relevant documentation
- [ ] I've added or updated tests where applicable
- [ ] I've reviewed my own diff for obvious issues
- [ ] Breaking changes are documented (if any)

## Notes for Reviewers

<!-- Optional: anything you'd like reviewers to pay particular attention to. -->
